...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀:
// nested function can access the variable internal because that variable is in the nested function's outer scope, therefore it has access.
...
function lowerCaseNames(data) {
  const newArray = data.map((item) => item.animal_name.toLowerCase());
  return newArray;
}
...
function lowPopulationAnimals(data) {
  const filteredArray = data.filter((item) => item.population < 5);
...
function USApop(data) {
  return data.reduce(function (acc, currentItem) {
    return (acc += currentItem.population);
  }, 0);
}
...
const cuboidTwo = new CuboidMakerTwo({ height: 5, width: 5, length: 5 });
